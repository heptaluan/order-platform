  ...
  <div class="p-4">
    ...
    >
      ...
      <template #toolbar>
        ...
        <a-button @click="handleExpressExport">导出快递取样表</a-button>
        <a-button @click="handleSamplesExport">导出实际取样表</a-button>
      </template>
    </BasicTable>
  </div>
</template>
...
import {
  ...
  getExpressExportColumns,
  getOrderSummaryColumns,
  getOrderConfirmExportColumns
} from './../tableData';
import { getExportOrder } from '/@/api/order/order';
...
export default defineComponent({
  ...
  setup() {
    ...
    function handleExport() {
      ...
      const Columns = getOrderExportColumns()
      const fileNamePrefix = '订单统计表'
      exportExcel(Columns, fileNamePrefix)
    }
    function handleExpressExport() {
      if (checkedKeys.value.length === 0) {
        return false
      }
      const Columns = getExpressExportColumns()
      const fileNamePrefix = '快递取样表'
      exportExcel(Columns, fileNamePrefix)
    }
    function handleSamplesExport() {
      if (checkedKeys.value.length === 0) {
        return false
      }
      const Columns = getOrderConfirmExportColumns()
      const fileNamePrefix = '实际取样表'
      exportExcel(Columns, fileNamePrefix)
    }
    function exportExcel(Columns, fileNamePrefix) {
      const rows = getSelectRows()
      const FieldsValue = getForm().getFieldsValue()
      const date = FieldsValue.date || dayjs().format('YYYY-MM-DD')
