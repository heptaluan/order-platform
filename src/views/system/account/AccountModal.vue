    ...
    setup(_, { emit }) {
      ...
      const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
          ...
          isHealthCommissionRole.value = data.record.roleIdList ? data.record.roleIdList.includes('5') : false
      ...
      async function handleSubmit() {
        try {
          ...
          closeModal();
          emit('success', { isUpdate: unref(isUpdate), values: { ...values, id: rowId.value }, isUserNameModify });
        } finally {
          setModalProps({ confirmLoading: false });
        }
      }
